...
Usage (from the project root):
    python scripts/recalculate_cash_bills.py --month 4 --year 2026 --mrp-drop-percent 10
    python scripts/recalculate_cash_bills.py --month 4 --year 2026 --mrp-drop-percent 10 --apply
"""
...
def apply_bill_updates(db, bill, recalculated_items):
    ...
    original_advance  = round2(bill["advance_paid"] or 0)
    # Cap advance_paid to the new total so the bill remains settled after repricing.
    advance_paid  = min(original_advance, new_final_total)
    ...
    db.executemany(
        ...
        [
            (
                item["mrp"], item["line_total"], item["discount_amount"],
                item["rate_after_disc"], item["final_amount"], item["id"],
            )
    ...
    db.execute(
        ...
        (new_subtotal, new_total_discount, new_final_total, new_total_discount,
         advance_paid, new_remaining, bill["id"]),
    )

    # Cash bills keep a single cash payment matching the bill total.
...
def main():
    ...
    with closing(get_db()) as db:
        ...
        for bill in bills:
            ...
            recalculated_items = [recalculate_item(item, args.mrp_drop_percent) for item in items]

            try:
                if args.apply:
                    totals = apply_bill_updates(db, bill, recalculated_items)
                else:
                    ft = round2(sum(item["final_amount"] for item in recalculated_items))
                    ap = min(round2(bill["advance_paid"] or 0), ft)
                    totals = {
